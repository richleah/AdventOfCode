...
namespace AdventOfCode.Aoc2021
{
    public class Day_12
    {
        ...
        {
            // Arrange
            var caveMap = CaveMapLoader.Load(inputData);
    ...
    public class CaveMapPathFinder
    {
        ...
        {
            var isAnyPreviouslyVisitedSmallCageBeenVisitedTwice = previouslyVisitedCavePath.Where(x => x.CanOnlyBeVisitedOnce).GroupBy(y => y.Name).Any(z => z.Count() == 2);
            ...
            var caveThatWasPopped = previouslyVisitedCavePath.Pop();
    ...
    public class CaveMapLoader
    {
        public static CaveMap Load(string? data)
        {
            ...
            foreach (string lineData in inputData)
            {
                ...
                var caveConnections = lineData.Split("-", StringSplitOptions.None | StringSplitOptions.RemoveEmptyEntries);
                var caveAName = caveConnections[0];
                var caveBName = caveConnections[1];
    ...
    {
        private Dictionary<string, CaveNode> _caveList;
